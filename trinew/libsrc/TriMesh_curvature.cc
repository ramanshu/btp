...
void TriMesh::prune_points(float pruning_threshold)
{
			/*ramanshu */
			int ct=0;
			for (int i = 0; i < vertices.size(); i++)
			{
			pruned[i] = (fabs(curv2[i]/curv1[i])> pruning_threshold);
			//pruned[i] = 0;//(fabs(curv2[i]/curv1[i])> pruning_threshold);

//			cout << pruned[i] << " " <<curv2[i] <<" "<< curv1[i] << " "<< curv2[i]/curv1[i]<<endl;
			if(pruned[i])
			ct++;
			}
			num_valid_points=vertices.size()-ct;
			/*ramanshu */
}
