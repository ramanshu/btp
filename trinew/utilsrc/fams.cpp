...
int FAMS::LoadPoints(Transformation_list *list,float diagonal)
{
//   bgLog("Load data points from %s...",filename);
   CleanPoints();
/*
...
//   for (i=0; (i<(n_*d_)) && (fscanf(fd, "%g", &pttemp[i]) == 1); i++);
     for (i=0; i< n_ ; i++)
		{
			pttemp[ct++]=(list->at(i))->s*180;
			pttemp[ct++]=(list->at(i))->r[0]*10*180/M_PI;
			pttemp[ct++]=(list->at(i))->r[1]*10*180/M_PI;
			pttemp[ct++]=(list->at(i))->r[2]*10*180/M_PI;
			pttemp[ct++]=(list->at(i))->t[0]*180*10/diagonal;
			pttemp[ct++]=(list->at(i))->t[1]*180*10/diagonal;
			pttemp[ct++]=(list->at(i))->t[2]*180*10/diagonal;
		}
//   fclose(fd);
/*
   if (i!= (n_*d_))
...
Transformation_list* FAMS::SavePrunedModes()
{
   Transformation_list *modes_list = new Transformation_list();   
   if (npm_ < 1)
      return NULL;
//   bgLog("Save joined convergence points in %s ...", fn);
   ...
   for (i=0; i<npm_; i++)
   {
         ...
         temp->s=prunedmodes_[idx++]*(maxVal_-minVal_)/65535.0 + minVal_;
         temp->r[0]=prunedmodes_[idx++]*(maxVal_-minVal_)/65535.0 + minVal_;
         temp->r[1]=prunedmodes_[idx++]*(maxVal_-minVal_)/65535.0 + minVal_;
         temp->r[2]=prunedmodes_[idx++]*(maxVal_-minVal_)/65535.0 + minVal_;
         temp->t[0]=prunedmodes_[idx++]*(maxVal_-minVal_)/65535.0 + minVal_;
         temp->t[1]=prunedmodes_[idx++]*(maxVal_-minVal_)/65535.0 + minVal_;
         temp->t[2]=prunedmodes_[idx++]*(maxVal_-minVal_)/65535.0 + minVal_;
         modes_list->push_back(temp);         
   }
//   bgLog("done\n");
   return modes_list;
}
...
Transformation_list* clusterTranformationData(int K,int L,int k,Transformation_list *list,float diagonal,int jump,float percent,float width, float epsilon,int Kmin,int Kjump)
{
...
//   char *data_file_name, *input_path;
 ...
 //  int Kmin, Kjump, 
   ...
   if (cfams.LoadPoints(list,diagonal))
      return NULL;
   ...
   cfams.RunFAMS(K, L, k_neigh, percent, jump, width, "pilot_100_monkeyData.txt");

   // save the data
   //sprintf(fdata_file_name,"%sout_%s.txt",input_path,data_file_name);
   Transformation_list *modes_list;
   cfams.SaveModes("out_100_monkeyData.txt");
   // save pruned modes modes
   //sprintf(fdata_file_name,"%smodes_%s.txt",input_path, data_file_name);
	modes_list = cfams.SavePrunedModes();
   return modes_list;
}
