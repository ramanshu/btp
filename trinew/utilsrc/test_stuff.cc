...
#include <string>
#include "myicp.h"
#include "reflect_model.cc"
#include "combine_models.cc"
using namespace std;
...
int current_mesh = -1;
bool check_firsts=true;
bool check_seconds=true;
bool check_draw_tstripes=true;
bool check_draw_matchings=true;
...
void draw_clusters(const TriMesh *themesh)
{
...
{  
  glLineWidth(2);
    for(int i=0;i<firsts[currentCluster].size();i++)  
   {
        glColor4f( 0.5f, 0.5, 0.5f, 0.1f );
    glBegin(GL_LINES);
    glVertex3f( themesh->vertices[firsts[currentCluster][i]][0],themesh->vertices[firsts[currentCluster][i]][1],themesh->vertices[firsts[currentCluster][i]][2] );    
    glVertex3f( themesh->vertices[seconds[currentCluster][i]][0],themesh->vertices[seconds[currentCluster][i]][1],themesh->vertices[seconds[currentCluster][i]][2] );
    glEnd();
   }}
}
...
void draw_mesh(int i)
{
	...
	if (draw_edges) {
		...
		glEnable(GL_POLYGON_OFFSET_FILL);
         ...
         if(check_draw_tstripes)
	draw_tstrips(themesh);
        draw_clusters(themesh);
...
void changeCluster(TriMesh *mesh,int m)
{ 
  if(m)
  currentCluster++;
  else
 ...
 if(currentCluster<0 || currentCluster>=firsts.size())
  {
  currentCluster=-1;
  ...
  else 
  {
	  for(int i=0;i<firsts[currentCluster].size();i++)
	  {
 	mesh->onCluster[firsts[currentCluster][i]]=1;
	mesh->onCluster[seconds[currentCluster][i]]=-1;
	  }
	...
	switch (key) {
		...
		case '@': // Shift-2
			draw_2side = !draw_2side; break;
                case 'a':
			changeCluster(mesh,0); 
                       break;
                case 'b':
			changeCluster(mesh,1); 
                       break;
                case 'n':
			check_draw_tstripes=!check_draw_tstripes; 
                       break;
                case 'm':
			check_draw_matchings=!check_draw_matchings; 
                       break;
		case 'e':
			draw_edges = !draw_edges; break;
		case 'i':
			check_firsts = !check_firsts; break;
		case 'o':
			check_seconds = !check_seconds; break;
		case 'f':
...
int main(int argc, char *argv[])
{
	...
	option =0; 
	int num_points;// = mesh ->vertices.size();
        int numvertices;//=mesh->vertices.size();
	
	if(option == 1){
	cout << "OPTION == 1" << endl;
	TriMesh *temp_model1= TriMesh::read(argv[1]);
	num_points = temp_model1 ->vertices.size();
	mesh_scale(temp_model1,-1,1,1);
        TriMesh *temp_model2 = TriMesh::read(argv[1]);
        vector< TriMesh* >* mesh_list = new vector< TriMesh* >();
        mesh_list->push_back(temp_model1);
        mesh_list->push_back(temp_model2);
        mesh= combine_models(mesh_list);
        numvertices=mesh->vertices.size();
        mesh->onCluster.resize(mesh->vertices.size());
...
/*	meshes.push_back(mesh);
	...
	*/
	Transformation::initialize();
		/*Transformation temp0(mesh,0,507);
		temp0.printTrans();
		cout << "P0 " << mesh->vertices[0][0] << " " << mesh->vertices[0][1] << " " << mesh->vertices[0][2] << endl;
		cout << "P507 " << mesh->vertices[507][0] << " " << mesh->vertices[507][1] << " " << mesh->vertices[507][2] << endl;
		point p=applyTransformation(&temp0,&(mesh->vertices[0]));
		cout << "result p " << p[0] << " " << p[1] << " "<< p[2] << endl;		
		Transformation temp1(mesh,1,508);
		Transformation temp2(mesh,2,509);
		temp1.printTrans();
		temp2.printTrans();
		Transformation temp3(mesh,3,9);
		Transformation temp4(mesh,4,10);
		Transformation temp5(mesh,5,11);*/
	//cout <<"Here"<<endl;
      mesh->prune_points(.75);
       int num_valid_vertices=mesh->num_valid_points;
       printf("Valid %d of %d\n",num_valid_vertices,mesh->vertices.size());
	 Transformation_list *list; 
       list=transformation_list(mesh,.005,0);
      Transformation_list *normalized = normalize(list,mesh);
      FILE *fd = fopen("monkeyData.txt","w");
      print_to_file(fd,normalized);
      fclose(fd);
    Transformation_list* modes_list = clusterTranformationData(40,40,100,list, mesh->need_diagonal_length(),0,0,-1, 0.05,1,1);
    for (Transformation_list::iterator it=modes_list->begin();it!=modes_list->end();it++)
   	{
		(*it)->printTrans();
   	}   	
	vector< vector<pair<int,int> >* >* pts = growCLuster_ICP(list,normalized, modes_list, mesh);
	cout << "PTS _> size = " << pts->size() << endl;
		firsts.resize(pts->size());
    seconds.resize(pts->size());
    int i = 0;
    for (vector< vector<pair<int,int> >* >::iterator it=pts->begin();it!=pts->end();it++) {
	    for (vector<pair<int,int> >::iterator it1=(*it)->begin();it1!=(*it)->end();it1++) {
	    	if((*it1).first <num_points && (*it1).second >= num_points)
	    	{firsts[i].push_back((*it1).first );
    		seconds[i].push_back((*it1).second- num_points);
    		}
	    	if((*it1).first >=num_points && (*it1).second < num_points)
	    	{firsts[i].push_back((*it1).first - num_points);
    		seconds[i].push_back((*it1).second);
    		}
	    	if((*it1).first < num_points && (*it1).second < num_points)
	    	{firsts[i].push_back((*it1).first);
    		seconds[i].push_back((*it1).second);
